...
def generate(image, mc_resolution, do_bake_texture=False, formats=["obj", "glb"]):
    scene_codes = model(image, device=device)
    # has_vertex_color=True always so colors are embedded in the mesh
...
def run_example(image_pil):
    ...
    mesh_name_obj, mesh_name_glb = generate(preprocessed, 256, formats=["obj", "glb"])
...
with gr.Blocks(title="TripoSR") as interface:
    ...
    **Tips:**
        ...
        with gr.Column():
            ...
            with gr.Row():
                with gr.Group():
                    ...
                    bake_texture = gr.Checkbox(
                        label="Bake Texture (Better Face Colors - Slow)",
                        value=False
                    )
            with gr.Row():
    ...
    submit.click(fn=check_input_image, inputs=[input_image]).success(
        ...
        fn=generate,
        inputs=[processed_image, mc_resolution, bake_texture],
